...
def resize_images_unlabelled(folder_path, new_imgs_folder="./Resized_All/Unlabelled", img_size=IMAGE_SIZE):
    # if the folder already exists, then return. Otherwise, create a new folder
    # with the resized images
    if os.path.exists(new_imgs_folder):
        if len(os.listdir(new_imgs_folder)) == 0:
...
if os.path.exists('./Resized_All'):
    if len(os.listdir('./Resized_All')) == 0:
        os.rmdir('./Resized_All')
else:
    os.mkdir('./Resized_All')
